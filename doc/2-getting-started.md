...
Create a new local vim configuration storage in your home directory (`~/.vim`), if it doesn't exist already. The `~/.vim/syntax` directory  contains the actual syntax file and the one in `~/.vim/ftdetect` is used to detect the file type. Both are called `icinga2.vim`. 
...
On Debian systems replace the path `/usr/share/doc/icinga2-common-[x.x.x]/syntax/vim` with `/usr/share/vim/addons`. Alternatively, you can use the `vim-addons` tool to create the `~/.vim/syntax` and `~/.vim/ftdetect` directories and create symlinks to the files in `/usr/share/vim/addons`:
...
Copy the `/etc/nanorc` sample file to your home directory and rename it to `~/.nanorc`. Create the `/etc/nano` directory and copy the provided `icinga2.nanorc` into it. 
...
![id command on CentOS 7](images/getting-started/id-apache-centos7.png "Confirm that the webserver's user (here: `apache` on a CentOS 7 system) is a member of the group `icingacmd`.")
